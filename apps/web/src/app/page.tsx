...
function PortfolioPreview() {
  return (
    <div className="relative">
      ...
      <div className="absolute -top-8 -right-8 w-48 h-48 bg-blue-400/15 dark:bg-blue-500/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute -bottom-6 -left-6 w-36 h-36 bg-violet-400/10 dark:bg-violet-500/8 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="relative bg-zinc-100/80 dark:bg-zinc-900/80 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/60 shadow-elevated overflow-hidden backdrop-blur-sm">
        ...
        <div className="px-4 py-3 border-b border-zinc-200/80 dark:border-zinc-800 flex items-center gap-3">
          ...
          <div className="flex-1 bg-white/60 dark:bg-zinc-800/60 rounded-md px-3 py-1 border border-zinc-200/60 dark:border-zinc-700/60">
            <span className="text-[10px] font-mono text-muted-foreground">alexmorgan.initmyfolio.com</span>
          </div>
          <ArrowSquareOut weight="regular" className="w-3.5 h-3.5 text-muted-foreground/50" />
        </div>
        ...
        <div className="bg-white dark:bg-zinc-950 p-5">
          ...
          <div className="flex items-start gap-3 mb-5">
            <div className="w-11 h-11 rounded-full bg-gradient-to-br from-blue-400 to-violet-500 shrink-0 shadow-sm" />
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-foreground">Alex Morgan</div>
              <div className="text-xs text-muted-foreground mt-0.5">Full-stack · San Francisco</div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            {[["47", "Repos"], ["2.4k", "Stars"], ["312", "Forks"]].map(([v, l]) => (
              <div key={l} className="bg-zinc-50 dark:bg-zinc-900/60 rounded-xl p-2.5 text-center border border-zinc-100 dark:border-zinc-800/60">
                <div className="text-sm font-bold text-foreground font-mono leading-tight">{v}</div>
          ...
          <div className="space-y-1.5 mb-4">
            {[
              { name: "neural-search", lang: "TypeScript", color: "#3178c6", stars: "2.4k" },
              { name: "pg-streamer", lang: "Go", color: "#00ADD8", stars: "847" },
              { name: "css-houdini-lab", lang: "JavaScript", color: "#f1e05a", stars: "203" },
            ].map(r => (
              <div key={r.name} className="flex items-center justify-between px-3 py-2 rounded-lg bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-100/80 dark:border-zinc-800/60 hover:bg-zinc-100/80 dark:hover:bg-zinc-900/60 transition-colors">
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: r.color }} />
                  <span className="text-xs font-medium text-foreground truncate">{r.name}</span>
                  <span className="text-[10px] text-muted-foreground hidden sm:block">{r.lang}</span>
                </div>
          ...
          <div className="space-y-1.5">
            <div className="h-1.5 rounded-full overflow-hidden flex gap-px">
              {[["#3178c6", "58%"], ["#00ADD8", "24%"], ["#f1e05a", "12%"], ["#dea584", "6%"]].map(([c, w]) => (
                <div key={c} style={{ backgroundColor: c, width: w }} className="first:rounded-l-full last:rounded-r-full" />
              ))}
            </div>
            <div className="flex gap-3 flex-wrap">
              {[["TypeScript", "#3178c6", "58%"], ["Go", "#00ADD8", "24%"]].map(([l, c, p]) => (
                <div key={String(l)} className="flex items-center gap-1">
                  <div className="w-2 h-2 rounded-full" style={{ backgroundColor: String(c) }} />
                  <span className="text-[10px] text-muted-foreground">{l} {p}</span>
                </div>
              ))}
            </div>
          </div>
...
function PricingCard({
  tier, eyebrow, price, period, features, cta, href, highlighted,
}: {
  tier: string; eyebrow: string; price: string; period: string;
  features: { text: string; included: boolean }[];
  cta: string; href: string; highlighted?: boolean;
}) {
  return (
    <div className={`relative rounded-2xl border p-8 flex flex-col gap-6 ${
      highlighted
      ...
      <div>
        <div className="text-xs font-medium text-muted-foreground mb-3">{eyebrow}</div>
      ...
      <ul className="space-y-3 flex-1">
        {features.map((f) => (
          <li key={f.text} className="flex items-start gap-2.5">
            {f.included ? (
              <CheckCircle weight="fill" className="w-4 h-4 text-primary shrink-0 mt-0.5" />
            ) : (
              <Lock weight="regular" className="w-4 h-4 text-muted-foreground/40 shrink-0 mt-0.5" />
            )}
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
        ...
        <nav className="max-w-5xl mx-auto bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border border-zinc-200/80 dark:border-zinc-800/50 rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.3)]">
          <div className="h-14 px-5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-6">
              <span className="font-semibold text-sm text-foreground tracking-tight">InitMyFolio</span>
        ...
        <div className="grid md:grid-cols-[1fr_400px] gap-16 lg:gap-24 items-center">
          ...
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/8 dark:bg-primary/15 border border-primary/15 dark:border-primary/20 mb-6 animate-fade-up">
              ...
              <span className="text-xs font-medium text-primary">GitHub Portfolio Generator</span>
            </div>
            ...
            <p className="mt-6 text-lg text-muted-foreground leading-relaxed max-w-[46ch] animate-fade-up delay-200">
              ...
              <code className="text-foreground bg-secondary px-1.5 py-0.5 rounded-md text-sm font-mono">username.initmyfolio.com</code>
      ...
      <section id="how-it-works" className="max-w-6xl mx-auto px-4 sm:px-6 py-24">
        <div className="mb-14">
          <div className="text-xs font-mono uppercase tracking-widest text-primary/70 mb-3">How it works</div>
        ...
        <div className="space-y-0 divide-y divide-border/60">
          {[
            ...
            >
              <div className="font-mono text-6xl font-bold text-border leading-none select-none">{step.n}</div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed md:max-w-[44ch]">{step.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="max-w-6xl mx-auto px-4 sm:px-6 pb-24">
        <div className="mb-14">
          <div className="text-xs font-mono uppercase tracking-widest text-primary/70 mb-3">Architecture</div>
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-foreground">
            Built for speed.
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-2 rounded-2xl border border-border bg-card p-8 shadow-card">
            <div className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-5">Sync engine</div>
            <h3 className="text-xl font-semibold text-foreground mb-3">Sync once. Serve forever.</h3>
            <p className="text-sm text-muted-foreground leading-relaxed mb-6 max-w-[50ch]">
              GitHub data is synced once and stored in PostgreSQL. Portfolio pages are served directly from the database — zero GitHub API calls at runtime. No rate limits, no latency.
            </p>
            <div className="flex items-center gap-0">
              {[["GitHub API", "sync"], ["PostgreSQL", "cache"], ["Portfolio", "serve"]].map(([l, r], i) => (
                <div key={l} className={`flex-1 px-4 py-3 bg-secondary/60 border border-border/60 text-center ${i === 0 ? "rounded-l-xl" : i === 2 ? "rounded-r-xl" : ""}`}>
                  <div className="text-xs font-medium text-foreground">{l}</div>
                  <div className="text-[10px] font-mono text-primary mt-0.5 uppercase tracking-wide">{r}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-8 shadow-card flex flex-col justify-between">
            <div className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-4">Setup time</div>
            <div>
              <div className="text-7xl font-bold tracking-tighter text-foreground leading-none mb-2">30<span className="text-3xl text-muted-foreground font-medium">s</span></div>
              <p className="text-sm text-muted-foreground">From GitHub OAuth to live portfolio page.</p>
            </div>
          </div>

          {/* Small card: auto sync */}
          <div className="rounded-2xl border border-border bg-card p-8 shadow-card flex flex-col justify-between">
            <div className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-4">Auto-sync</div>
            <div>
              <div className="text-7xl font-bold tracking-tighter text-foreground leading-none mb-2">8<span className="text-3xl text-muted-foreground font-medium">h</span></div>
              <p className="text-sm text-muted-foreground">Automatic refresh keeps your portfolio up to date.</p>
            </div>
          </div>

          {/* Large card: customization */}
          <div className="md:col-span-2 rounded-2xl border border-border bg-card p-8 shadow-card">
            <div className="text-xs font-mono uppercase tracking-widest text-muted-foreground mb-5">Customization</div>
            <h3 className="text-xl font-semibold text-foreground mb-3">Full control over your portfolio.</h3>
            <p className="text-sm text-muted-foreground leading-relaxed mb-6 max-w-[50ch]">
              Choose your accent color, layout variant, hero style, section order, and more. Set your availability status, add a tech stack, pin repos, and write a custom bio.
            </p>
            <div className="flex flex-wrap gap-2">
              {["Accent colors", "Layout variants", "Section order", "Custom links", "Tech stack", "Featured repo"].map(f => (
                <span key={f} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-secondary border border-border/60 text-xs font-medium text-foreground">
      ...
      <section id="pricing" className="max-w-6xl mx-auto px-4 sm:px-6 pb-24">
        <div className="mb-14">
          <div className="text-xs font-mono uppercase tracking-widest text-primary/70 mb-3">Pricing</div>
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-foreground">
        ...
        <div className="mt-5 max-w-3xl">
          <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-primary/5 dark:bg-primary/10 border border-primary/15">
            ...
            <span className="text-sm text-primary/80 font-medium">Pro launching soon — all accounts will receive a free trial on launch.</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-24">
        <div className="rounded-2xl border border-border bg-card p-10 md:p-16 shadow-card">
          <div className="grid md:grid-cols-[1fr_auto] items-center gap-8">
            <div>
              <div className="text-xs font-mono uppercase tracking-widest text-primary/70 mb-3">Ready?</div>
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-foreground mb-3">
                Live in 30 seconds.
              </h2>
              <p className="text-muted-foreground text-sm">No credit card. No configuration. Just GitHub.</p>
            </div>
            <Link
              href="/login"
      ...
      <footer className="border-t border-border/60">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <span className="font-semibold text-sm text-foreground">InitMyFolio</span>
