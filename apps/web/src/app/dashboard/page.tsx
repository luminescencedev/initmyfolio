  ...
  if (loading) {
    return (
      <div className="min-h-[100dvh] bg-background">
        ...
        <main
          id="main-content"
          className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-5"
        >
    ...
    <div className="min-h-[100dvh] bg-background">
      ...
      >
        ...
        <div className="rounded-2xl border border-border bg-card shadow-card overflow-hidden">
          ...
          {syncMessage && (
            <div
              className={`px-6 py-3 border-t border-border/60 flex items-center gap-2.5 ${
              ...
              <>
                <div className="hidden sm:grid grid-cols-[1fr_80px_60px_60px] border-b border-border/60 px-5 py-2.5 bg-secondary/30">
                  {["Name", "Lang", "Stars", "Forks"].map((h) => (
                    <span
                      key={h}
                      className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider last:text-right [&:nth-child(3)]:text-right"
                    >
                      {h}
                    </span>
                  ))}
            ...
            <div className="rounded-2xl border border-border bg-card shadow-card overflow-hidden">
              ...
              <div className="divide-y divide-border/60">
                {[
                  [
                    ...
                    s.accentColor ? (
                      <span className="flex items-center gap-1.5">
                        <span
                          className={`w-2 h-2 rounded-full shrink-0 ${ACCENT_DOT[s.accentColor] ?? "bg-foreground"}`}
                        />
                  ...
                  [
                    "Sections",
                    `${visibleSections.length} / ${ALL_SECTIONS.length} visible`,
                  ],
                ].map(([label, value]) => (
                  <div
                    key={String(label)}
